...
2. **Just-in-time 提供 Visual Companion**：不要一开始就提供。第一次遇到“看图比文字更清楚”的问题时，再用独立消息提供；如果用户同意，浏览器 tab 会自动打开。若整个过程没有视觉问题，就不要提供。见下方 Visual Companion。
...
9. **进入实施规划**：调用 `superpowers-writing-plans`。
...
digraph brainstorming {
    ...
    "用户审阅 spec 吗？" -> "调用 superpowers-writing-plans" [label="已批准"];
}
```

**流程的终态是调用 `superpowers-writing-plans`。** 不要调用 frontend-design、mcp-builder 或其他实现 skill。Brainstorming 之后唯一该调用的 skill 是 `superpowers-writing-plans`。
...
**何时提出启用（just-in-time）：**
不要一开始就提供。等到某个问题真的“看图比文字更清楚”时，再单独发一条消息。它必须是实际 mockup、layout 或 diagram 问题，而不是仅仅“话题是 UI”。

> “接下来这部分如果能直接看图会更清楚。我可以边聊边在浏览器 tab 里展示 mockup、diagram 和对比稿。这个能力还比较新，而且会更耗 token。要我打开给你看吗？”

**这必须是一条独立消息。** 只发送这个 offer，不要混入澄清问题、总结或其他内容。等待用户响应。如果用户同意，用 `--open` 启动服务，让浏览器自动打开第一屏。如果用户拒绝，就继续纯文本；除非用户主动提起，不要再次提供。
