...
**提示：** 如果当前平台支持子代理，那么 `Superpowers` 在 `superpowers-subagent-driven-development` 模式下通常质量更高。支持子代理的平台包括 Claude Code、Codex CLI、Codex App 和 Copilot CLI；平台细节见 `../superpowers-using-superpowers/references/`。如果子代理可用，优先使用 `superpowers-subagent-driven-development`，不要使用本 skill。
