...
check_feature_branch() {
    ...
    if [[ ! "$branch" =~ ^[0-9]{3}- ]]; then
        if [[ -n "$repo_root" ]]; then
            local suggested_name=$(normalize_branch_to_feature_name "$branch" "$repo_root")
            echo "[specify] Notice: Branch '$branch' doesn't follow naming convention." >&2
            echo "[specify] Using '$suggested_name' as spec directory name." >&2
        fi
    fi
...
normalize_branch_to_feature_name() {
    ...
    local next_number=$(get_next_feature_number "$repo_root")
    echo "${next_number}-${branch}"
    return 0
}
...
get_feature_paths() {
    ...
    # Try prefix-based lookup first to find existing spec directory
    local feature_dir=$(find_feature_dir_by_prefix "$repo_root" "$current_branch")

    # If no existing spec directory found, use normalized feature name
    if [[ ! -d "$feature_dir" ]]; then
        local feature_name=$(normalize_branch_to_feature_name "$current_branch" "$repo_root")
